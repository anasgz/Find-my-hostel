...
from Backend.models import admindb, categorydb, productdb, admincontactdb
from django.core.files.storage import FileSystemStorage
from django.utils.datastructures import MultiValueDictKeyError
from django.contrib.auth import authenticate, login
from django.contrib.auth.models import User

from frondend.models import checkoutdb
...
def saveproduct(req):
    if req.method== "POST":
        ...
        IMAGE = req.FILES['image']
        obj=productdb(Name=NAME,Price=PRICE,Discription=DISCRIPTION,CATEGORY=CATEGORY,Quantity=QUANTITY,Image=IMAGE,Location=Loc)
        obj.save()
...
def updateproduct(req,dataid):
    if req.method=="POST":
        ...
        productdb.objects.filter(id=dataid).update(Name=NAME,Price=PRICE,Discription=DISCRIPTION,CATEGORY=CATEGORY,Quantity=QUANTITY,Image=file,Location=Loc)
...
def bookings(req):
    ...
    return render(req,"displaybookings.html", {'data':data})
def deletebookings(req,dataid):
